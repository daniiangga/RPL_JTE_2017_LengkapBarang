		...
		<div class="widget-content">
			...
			<div class="shop-options clearfix">
				...
				<form class="element-form" method="POST" action="<?php echo AJAX_URL; ?>">
					...
					<a href="#" class="element-button element-submit" data-title="<?php _e('Difavorit', 'makery'); ?>"><?php _e('Tidak Difavorit', 'makery'); ?></a>
					<input type="hidden" name="user_action" class="toggle" value="remove_relation" data-value="add_relation" />
					<?php } else { ?>
					<a href="#" class="element-button element-submit active" data-title="<?php _e('Tidak Difavorit', 'makery'); ?>"><?php _e('Difavorit', 'makery'); ?></a>
					...
					<a href="#contact_form" class="element-button element-colorbox square secondary" title="<?php _e('Tanya pertanyaan', 'makery'); ?>"><span class="fa fa-comment"></span></a>
					<?php } ?>
					<?php if(!ThemexCore::checkOption('shop_reports')) { ?>
					<a href="#report_form" class="element-button element-colorbox square secondary" title="<?php _e('Kirim Laporan', 'makery'); ?>"><span class="fa fa-flag"></span></a>
					<?php } ?>
					...
					<div id="report_form">
						<div class="site-popup medium">
							<form class="site-form element-form" method="POST" action="<?php echo AJAX_URL; ?>">
								...
								<div class="field-wrap">
									<textarea name="reason" cols="30" rows="5" placeholder="<?php _e('Alasan', 'makery'); ?>"></textarea>
								</div>
				...
				<?php } else { ?>
				<?php if(!ThemexCore::checkOption('shop_favorites')) { ?>
				<a href="<?php echo ThemexCore::getURL('register'); ?>" class="element-button active"><?php _e('Favoritekan', 'makery'); ?></a>
				<?php } ?>
				<?php if(!ThemexCore::checkOption('shop_questions')) { ?>
