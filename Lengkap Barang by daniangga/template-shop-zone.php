...
<div class="column fivecol">
	...
	<div class="element-title indented">
		...
		<h1><?php _e('Ubah zona', 'makery'); ?></h1>
		<?php } else { ?>		
		<h1><?php _e('Tambah zona', 'makery'); ?></h1>
		<?php } ?>
	</div>
	<?php if(!ThemexWoo::isActive() || ThemexCore::checkOption('shop_shipping') || !ThemexWoo::isShipping()) { ?>
	<span class="secondary"><?php _e('Toko ini tidak ada lagi.', 'makery'); ?></span>
	<?php } else if(empty($zone)) { ?>
	<span class="secondary"><?php _e('Zona ini tidak ada.', 'makery'); ?></span>
	<?php } else { ?>
					...
					<td>
						...
						echo ThemexInterface::renderOption(array(
							...
							'attributes' => array(
								...
								'multiple' => 'multiple',
								'data-placeholder' => __('Pilih opsi', 'makery'),
							),
						));
						?>
					</td>
		...
		?>
			...
			<table class="profile-fields">
				<tbody>
					...
					<tr>
						<th><?php _e('Minimal jumlah', 'makery'); ?></th>
						<td>
							<div class="field-wrap">
						...
						<td>
							<div class="element-select">
								...
								echo ThemexInterface::renderOption(array(
									...
									'type' => 'select',
									'options' => array(
										'yes' => __('Diaktifkan', 'makery'),
										'no' => __('Dimatikan', 'makery'),
									),
			...
			<table class="profile-table halved">
				<thead>
					<tr>
						<th><?php _e('Tipe berbelanja', 'makery'); ?></th>
						<th><?php _e('Biaya', 'makery'); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr> 
						<td><?php _e('Semua tipe', 'makery'); ?></td>
						<td>
							<div class="field-wrap">
		...
		<div id="zone_form">
			<div class="site-popup small">
				<form class="site-form" method="POST" action="">
					<p><?php _e('Apakah kamu ingin menghapus permanen zona ini?', 'makery'); ?></p>
					<a href="#" class="element-button element-submit primary"><?php _e('Hapus zona', 'makery'); ?></a>
					<input type="hidden" name="zone_id" value="<?php echo $zone['id']; ?>" />
					<input type="hidden" name="shop_id" value="<?php echo ThemexShop::$data['ID']; ?>" />
